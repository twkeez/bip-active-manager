...
export default function ClientReportPage({ report, draft }: Props) {
  ...
  // Performance Overview: flatten all channel metrics that have real data
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main ref={mainRef} className="mx-auto max-w-4xl space-y-8 px-8 py-6 text-gray-800">
        ...
        {isVisible("kpis") && (hasGsc || hasAds || hasGa4) && (
          <section>
            ...
            {hasGsc && (
              <div className="mb-6">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  {gscMetrics.map((m) => <MetricCard key={m.label} metric={m} />)}
                </div>
              </div>
            )}
            ...
            {hasGa4 && (
              <div className="mb-6">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  {ga4Metrics.map((m) => <MetricCard key={m.label} metric={m} />)}
                </div>
              </div>
            )}

            {/* Ads headline metrics */}
            {hasAds && (
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-3">
                  Google Ads
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  {adsMetrics.map((m) => <MetricCard key={m.label} metric={m} />)}
                </div>
              </div>
            )}
            ...
            <div className="rounded-2xl border border-gray-200 px-6 py-5">
              <div className="space-y-2.5">
                {report.gscTopPages.map((page) => {
                  ...
                  return (
                    <div key={page.page_url} className="grid items-center gap-3" style={{ gridTemplateColumns: "minmax(0,1.8fr) minmax(0,1fr) 44px" }}>
                    ...
                    return (
                      <tr key={row.keyword} className="border-b border-gray-100 last:border-0">
                        <td className="px-5 py-3 pr-4">
                          ...
                          {row.tag && <span className="ml-2 text-[10px] text-gray-400">{row.tag}</span>}
                        </td>
                        ...
                        <td className="px-5 py-3 text-right text-xs text-gray-500">
                          ...
                          {row.previousClicks > 0 && (
                            <span className="ml-1 text-gray-400">/ {row.previousClicks.toLocaleString()}</span>
                          )}
                        </td>
                      </tr>
                    );
