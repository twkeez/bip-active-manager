...
import type { AdsSignal, AdsSnapshot, BasecampSyncState, BasecampThreadEvent, ClientRow, ClientKeywordTarget, ClientReportingMetricPreference, GscPageMetric, GscQueryMetric, GscSignal, GscSnapshot, GbpReviewRow, GbpSnapshot, LighthouseAuditItem, LighthouseAuditOccurrence, LighthouseOccurrenceOverride, LighthouseSnapshot, KeywordHealthRow, ReportingAlertItem, ReportingFreshnessItem, ReportingKpiCard, SocialConnection, SocialDailySnapshot, SocialIdea, SocialPostSnapshot, SocialSignal, StrategistSummaryResult, SitemapSnapshot, SitemapUrlRow, SeoCrawlIssue, SeoCrawlSnapshot,
...
type DetailTab = |"profile" |"onboarding" |"connections" |"comms" |"reporting" |"seo_ops" |"seo" |"ads" |"sitemaps" |"social" |"actions";
...
return ( <div className="space-y-4"> <div className="rounded-lg border border-white/[0.08] bg-bip-page p-3"> <div className="flex items-center gap-2 text-sm font-medium text-white"> <ListTodo className="h-4 w-4" /> Weekly action queue </div> <p className="mt-1 text-xs text-white/75"> Prioritized issues from SEO, Ads, Sitemaps, and Social in one list. </p> </div> {prioritized.length === 0 ? ( <p className="rounded-lg border border-white/[0.08] bg-bip-page px-3 py-2 text-sm text-white/75"> No open technical actions for this client. </p> ) : ( <ul className="space-y-2"> {prioritized.map((finding) => ( <li key={finding.id}> <FindingCard finding={finding} /> </li> ))} </ul> )} <div className="rounded-lg border border-white/[0.08] bg-bip-page p-3"> <div className="flex items-center gap-2 text-sm font-medium text-white"> <BadgeAlert className="h-4 w-4" /> Rollout phases </div> <ul className="mt-2 space-y-1 text-xs text-white/75"> <li>Phase 1: Foundation UI in right panel and table.</li> <li>Phase 2: SEO and Sitemaps data sources.</li> <li>Phase 3: Ads and Social diagnostics.</li> <li>Phase 4: Assignment workflow maturity.</li> </ul> </div> </div> );
}
