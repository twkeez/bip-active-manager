...
type Props = { initialTasks: UserTaskWithSource[]; initialCategories: UserTaskCategory[]; clientOptions: TaskClientOption[]; initialPeople: UserTaskPerson[]; initialProjects: ClientProjectWithMeta[]; userEmail: string | undefined; isAdmin?: boolean; initialEmailMessages: UserEmailMessageRow[]; initialEmailRules: UserEmailSenderRule[];
};
...
export default function MyTasksManager({ initialTasks, initialCategories, clientOptions, initialPeople, initialProjects, userEmail, isAdmin = false, initialEmailMessages, initialEmailRules,
}: Props) {
...
const [senders, setSenders] = useState<Array<{ email: string; name: string | null }>>([]);
const [sendersLoading, setSendersLoading] = useState(false);
const [digestGroups, setDigestGroups] = useState<Array<{ category: string; label: string; count: number }>>([]);
const [digestSummaries, setDigestSummaries] = useState<Record<string, string>>({});
...
const payload = (await response.json()) as { connected?: boolean; expiresAt?: string | null; lastSyncedAt?: string | null; };
if (!response.ok) return;
setGmailConnected(Boolean(payload.connected));
setGmailConnectMeta( payload.connected ? `Connected${ payload.lastSyncedAt ? ` · last synced ${new Date(payload.lastSyncedAt).toLocaleString()}` :" · never synced" }` :"Not connected", ); }
...
async function handleSyncGmail(full = false) {
...
async function handleUpsertGmailRule( sender: string, ruleType:"blacklist" |"always_high_priority", isActive = true, ) {
...
async function loadSenders() {
  ...
  try {
    const response = await fetch("/api/gmail/senders");
    const payload = (await response.json()) as { error?: string; senders?: Array<{ email: string; name: string | null }>; };
...
async function loadDigestGroups() {
  setDigestLoading(true);
  try {
    const response = await fetch("/api/gmail/digest");
    const payload = (await response.json()) as { error?: string; groups?: Array<{ category: string; label: string; count: number }>; };
    if (!response.ok) throw new Error(payload.error ?? "Failed to load digest groups");
    setDigestGroups(payload.groups ?? []);
  } catch (err) {
    setError(err instanceof Error ? err.message : "Failed to load digest groups");
  } finally {
    setDigestLoading(false);
  }
}
async function summarizeDigestGroup(category: string) {
  setSummarizingCategory(category);
  try {
    const response = await fetch("/api/gmail/digest", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ category }) });
    const payload = (await response.json()) as { error?: string; summary?: string; };
    if (!response.ok) throw new Error(payload.error ?? "Failed to summarize group");
    setDigestSummaries((prev) => ({ ...prev, [category]: payload.summary ?? "" }));
  } catch (err) {
    setError(err instanceof Error ? err.message : "Failed to summarize group");
  } finally {
