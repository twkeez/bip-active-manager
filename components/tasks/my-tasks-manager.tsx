...
import { ArrowLeft, Brain, CalendarClock, Check, Copy, FolderPlus, Loader2, Mail, MessageCircle, Search, Send, Star, Trash2, UserRound,
} from "lucide-react";
...
const [triageResult, setTriageResult] = useState<{ doToday: string[]; stalled: string[]; canWait: string[] } | null>(null);
...
async function handleTriage() {
  ...
  setError(null);
  try {
    const res = await fetch("/api/tasks/ai-triage", { method: "POST" });
    const data = (await res.json()) as { error?: string; triage?: { doToday: string[]; stalled: string[]; canWait: string[] } };
...
return ( <div className="min-h-screen bg-bip-page text-white font-sans" > <div className="mx-auto flex min-h-screen w-full max-w-7xl gap-3 px-4 py-4 md:px-6"> <aside className="w-56 shrink-0 rounded-xl border border-white/[0.08] bg-bip-card p-3 text-white/75 shadow-none"> <div className="mb-3 flex items-center justify-between"> <div> <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-white/50"> Wunderlist </p> <p className="text-xs text-white/50">{userEmail ??"Signed in"}</p> </div> <a href="/dashboard" className="rounded-md border border-white/[0.08] p-1 hover:bg-white/[0.06]"> <ArrowLeft className="h-3.5 w-3.5" /> </a> </div> <nav className="space-y-1"> {folders.map((item) => ( <button key={item.id}

type="button" onClick={() => setFolder(item.id)} className={`flex w-full items-center justify-between rounded-md px-2 py-1.5 text-[13px] transition ${ folder === item.id ?"bg-white/[0.12] text-white shadow-inner" :"text-white/75 hover:bg-white/[0.06] hover:text-white" }`} > <span className={`tracking-[0.01em] ${folder === item.id ?"font-semibold" :"font-medium"}`} > {item.label} </span> <span className="text-[11px] text-white/50">{item.count}</span> </button> ))} </nav> <div className="mt-3 rounded-md border border-red-500/80 bg-[#e0382f] px-2 py-1.5 text-xs font-semibold text-white shadow-none"> Overdue: {overdueCount} </div> <div className="mt-3 space-y-2 border-t border-white/[0.08] pt-3"> <p className="text-[11px] uppercase tracking-wide text-white/50">Email capture</p> {!forwardingAddress ? ( <button type="button" onClick={() => void handleLoadForwardingAddress(false)} className="w-full rounded-md border border-white/[0.08] bg-bip-card/60 px-2 py-1.5 text-xs hover:bg-white/[0.06]" > {tokenLoading ?"Loading..." :"Load forwarding address"} </button> ) : ( <button type="button" onClick={() => void navigator.clipboard.writeText(forwardingAddress)} className="flex w-full items-center gap-1 rounded-md border border-white/[0.08] bg-bip-card/60 px-2 py-1.5 text-left text-xs hover:bg-white/[0.06]" > <Copy className="h-3.5 w-3.5 shrink-0" /> <span className="truncate">{forwardingAddress}</span> </button> )} <button type="button" onClick={() => void handleLoadForwardingAddress(true)} className="w-full rounded-md border border-white/[0.08] bg-bip-card/60 px-2 py-1.5 text-xs hover:bg-white/[0.06]" > Rotate </button> {tokenMessage && <p className="text-[11px] text-white/50">{tokenMessage}</p>} </div> </aside> <main className="min-w-0 flex-1 rounded-xl border border-white/[0.08] bg-bip-card p-3 shadow-none"> <div className="mb-3 flex gap-1 rounded-lg border border-white/[0.08] bg-bip-card p-1"> <button type="button" onClick={() => { setMainView("tasks"); syncProjectsUrl("tasks", null); }} className={`flex-1 rounded-md px-3 py-1.5 text-sm font-medium transition ${ mainView ==="tasks" ?"bg-bip-card text-white shadow-none" :"text-white/50 hover:text-white/75" }`} > Tasks </button> <button type="button" onClick={() => { setMainView("projects"); syncProjectsUrl("projects", selectedProjectId); }} className={`flex-1 rounded-md px-3 py-1.5 text-sm font-medium transition ${ mainView ==="projects" ?"bg-bip-card text-white shadow-none" :"text-white/50 hover:text-white/75" }`} > Projects </button> </div> {mainView ==="projects" ? ( <section className="flex min-h-[70vh] gap-3 rounded-xl border border-white/[0.08] bg-bip-card p-3 shadow-none"> <div className="w-full max-w-sm shrink-0 border-r border-white/10 pr-3"> <ProjectsPanel projects={projects} clientOptions={clientOptions} selectedProjectId={selectedProjectId} onSelectProject={selectProject} onProjectsChange={setProjects} onError={setError} /> </div> <div className="min-w-0 flex-1"> {selectedProjectId != null ? ( <ProjectWorkspace projectId={selectedProjectId} tasks={tasks} clientOptions={clientOptions} onProjectUpdated={handleProjectUpdated} onProjectsListRefresh={reloadProjects} onTasksRefresh={reloadTasks} onPatchTask={patchTask} onOpenTaskDetail={(task) => { setMainView("tasks"); syncProjectsUrl("tasks", null); openDetail(task); }} onProjectDeleted={async () => { const response = await fetch("/api/projects", { cache:"no-store" });
const payload = (await response.json()) as { projects?: ClientProjectWithMeta[]; };
const next = payload.projects ?? [];
setProjects(next);
const fallback = next[0]?.id ?? null;
setSelectedProjectId(fallback); syncProjectsUrl("projects", fallback); }} onError={setError} /> ) : ( <p className="flex h-full items-center justify-center text-sm text-white/40"> Select or create a project to get started. </p> )} </div> </section> ) : null} {mainView ==="tasks" ? ( <section className="rounded-xl border border-white/[0.08] bg-bip-card p-3 shadow-[0_1px_2px_rgba(0,0,0,0.18)]"> <div className="mb-2 flex items-center justify-between gap-2"> <h1 className="text-[22px] font-semibold tracking-[-0.01em] text-white/75"> {folders.find((item) => item.id === folder)?.label ??"Board"} </h1> <div className="flex items-center gap-1.5"> <label className="relative"> <Search className="pointer-events-none absolute left-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-white/40" /> <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search tasks" className="rounded-md border border-white/[0.12] bg-bip-card/78 py-1.5 pl-7 pr-2 text-[13px] text-white/75" /> </label> <select value={categoryFilter == null ?"" : String(categoryFilter)} onChange={(event) => setCategoryFilter(event.target.value ? Number(event.target.value) : null) } className="rounded-md border border-white/[0.12] bg-bip-card/78 px-2 py-1.5 text-[13px] text-white/75" > <option value="">All categories</option> {categories.map((category) => ( <option key={category.id} value={category.id}> {category.name} </option> ))} </select> {triageConfirm ? (
  <span className="inline-flex items-center gap-1.5 rounded-md border border-amber-500/40 bg-amber-500/10 px-2.5 py-1.5 text-[13px]">
    <span className="text-amber-300">Uses AI API (~$0.01 max)</span>
    <button type="button" onClick={() => { setTriageConfirm(false); void handleTriage(); }} className="font-semibold text-emerald-400 hover:text-emerald-300">Confirm</button>
    <span className="text-white/30">·</span>
    <button type="button" onClick={() => setTriageConfirm(false)} className="text-white/40 hover:text-white/70">Cancel</button>
  </span>
) : (
  <button type="button" onClick={() => setTriageConfirm(true)} disabled={triageLoading} className="inline-flex items-center gap-1.5 rounded-md border border-violet-500/40 bg-violet-500/10 px-2.5 py-1.5 text-[13px] font-medium text-violet-300 hover:bg-violet-500/20 disabled:opacity-60 transition" >
    {triageLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Brain className="h-3.5 w-3.5" />}
    {triageLoading ? "Thinking..." : "AI Triage"}
  </button>
)} </div> </div>
{triageResult && ( <div className="mb-3 rounded-lg border border-violet-500/20 bg-violet-500/[0.06] p-3"> <div className="mb-2 flex items-center justify-between"> <span className="text-[11px] font-semibold uppercase tracking-widest text-violet-300">AI Task Triage</span> <button type="button" onClick={() => setTriageResult(null)} className="text-[11px] text-white/40 hover:text-white/70">Dismiss</button> </div> <div className="grid gap-3 sm:grid-cols-3"> <div> <p className="mb-1 text-[10px] font-semibold uppercase tracking-widest text-emerald-400">Do Today ({triageResult.doToday.length})</p> <ul className="space-y-0.5"> {triageResult.doToday.map((t) => <li key={t} className="text-[12px] text-white/75">• {t}</li>)} {triageResult.doToday.length === 0 && <li className="text-[12px] text-white/40">Nothing urgent</li>} </ul> </div> <div> <p className="mb-1 text-[10px] font-semibold uppercase tracking-widest text-amber-400">Stalled ({triageResult.stalled.length})</p> <ul className="space-y-0.5"> {triageResult.stalled.map((t) => <li key={t} className="text-[12px] text-white/75">• {t}</li>)} {triageResult.stalled.length === 0 && <li className="text-[12px] text-white/40">None stalled</li>} </ul> </div> <div> <p className="mb-1 text-[10px] font-semibold uppercase tracking-widest text-white/50">Can Wait ({triageResult.canWait.length})</p> <ul className="space-y-0.5"> {triageResult.canWait.map((t) => <li key={t} className="text-[12px] text-white/75">• {t}</li>)} {triageResult.canWait.length === 0 && <li className="text-[12px] text-white/40">Nothing here</li>} </ul> </div> </div> </div> )}
 <div className="mb-3 rounded-lg border border-white/[0.08] bg-bip-card/70 p-2 shadow-[inset_0_1px_2px_rgba(0,0,0,0.16)]"> <div className="grid gap-2 md:grid-cols-[1fr_auto]"> <input value={title} onChange={(event) => setTitle(event.target.value)} onKeyDown={(event) => { if (event.key ==="Enter") { event.preventDefault(); void handleCreateTask(); } }} placeholder="Quick add task... press Enter" className="rounded-md border border-white/[0.12] bg-bip-card/85 px-3 py-2 text-[13px] text-white/75 placeholder:text-white/40" /> <button type="button" onClick={() => void handleCreateTask()} disabled={saving} className="rounded-md border border-bip-accent bg-bip-accent px-3 py-2 text-[13px] font-medium text-bip-page shadow-none disabled:opacity-60" > {saving ?"Adding..." :"Add"} </button> </div> <p className="mt-2 text-[11px] text-white/50"> Add quickly here, then click the task to assign client, label, due date, and other details in the popout. </p> </div> <div className="overflow-hidden rounded-lg border border-white/[0.08] shadow-[inset_0_1px_0_rgba(255,255,255,0.45)]"> {visibleTasks.length === 0 ? ( <p className="px-4 py-12 text-center text-sm text-white/50"> No tasks in this folder yet. </p> ) : folder ==="inbox" ? ( <div className="p-3"> <p className="mb-3 text-[11px] text-white/50"> Tasks are grouped by stage — scroll down and drag between sections to update status. </p> <TaskStageBoard tasks={visibleTasks} onMoveTask={(taskId, status) => void handleMoveTaskStage(taskId, status)} onToggleDone={(task) => void handleToggleDone(task)} onToggleStar={(task) => void handleToggleStar(task)} onOpenDetail={openDetail} onDeleteTask={(taskId) => void handleDeleteTask(taskId)} formatDate={formatDate} taskSourceLabel={taskSourceLabel} buildExternalSourceUrl={buildExternalSourceUrl} /> </div> ) : ( <div> {groupedVisibleTasks.map(([groupName, groupTasks]) => ( <section key={groupName}> <div className="border-b border-white/[0.08] bg-bip-page px-3 py-1.5"> <h3 className="text-[11px] font-semibold uppercase tracking-[0.08em] text-white/75"> {groupName} ({groupTasks.length}) </h3> </div> <ul> {groupTasks.map((task) => { const sourceUrl = buildExternalSourceUrl(task.source);
return ( <li key={task.id} className={`flex items-start gap-2.5 border-b border-white/[0.08] bg-bip-card px-3 py-2 transition-colors duration-150 hover:bg-white/[0.06] ${ task.is_starred ?"bg-amber-500/10" :"" }`} > <button type="button" onClick={() => void handleToggleDone(task)} className={`mt-0.5 inline-flex h-[19px] w-[19px] shrink-0 items-center justify-center rounded-full border transition-colors duration-150 ${ task.status ==="done" ?"border-emerald-500 bg-emerald-500 text-white shadow-[inset_0_1px_1px_rgba(255,255,255,0.25)]" :"border-white/30 bg-bip-card text-transparent hover:border-emerald-500" }`} aria-label={task.status ==="done" ?"Mark not done" :"Mark done"} > <Check className="h-3 w-3" /> </button> <button type="button" onClick={() => openDetail(task)} className="min-w-0 flex-1 text-left" > <p className={`truncate text-[14px] font-medium text-white/75 transition-all duration-200 ${ task.status ==="done" ?"text-white/40 line-through decoration-white/40" :"" }`} > {task.title} </p> <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[10px] text-white/50"> <span className="rounded-full border border-white/[0.12] px-2 py-0.5"> {taskStageLabelSafe(task.status)} </span> <span className="inline-flex items-center gap-1"> <CalendarClock className="h-3 w-3" /> {formatDate(task.due_date)} </span> <span className="rounded-full border border-white/[0.12] px-2 py-0.5"> {task.priority.toUpperCase()} </span> {task.category?.name && ( <span className="rounded-full border border-violet-500/20 bg-violet-500/10 px-2 py-0.5 text-violet-300"> {task.category.name} </span> )} {task.client && ( <span className="inline-flex items-center gap-1 rounded-full border border-white/[0.12] px-2 py-0.5"> <UserRound className="h-2.5 w-2.5" /> {task.client.account_name} </span> )} <span className="rounded-full border border-white/[0.12] px-2 py-0.5"> {taskSourceLabel(task)} </span> </div> {sourceUrl && ( <a href={sourceUrl} target="_blank" rel="noopener noreferrer" className="mt-1 inline-block text-[10px] text-white/50 underline" onClick={(event) => event.stopPropagation()} > Open source </a> )} </button> <button type="button" onClick={() => void handleToggleStar(task)} className={`mt-0.5 rounded p-1 transition-colors ${ task.is_starred ?"text-[#ff6a2f]" :"text-white/75 hover:text-orange-400" }`} aria-label="Toggle star" > <Star className={`h-3.5 w-3.5 ${task.is_starred ?"fill-current" :""}`} /> </button> <button type="button" onClick={() => void handleDeleteTask(task.id)} className="mt-0.5 rounded p-1 text-white/75 transition-colors hover:text-red-500" aria-label="Delete task" > <Trash2 className="h-3.5 w-3.5" /> </button> </li> ); })} </ul> </section> ))} </div> )} </div> <div className="mt-4 rounded-lg border border-white/[0.08] bg-bip-card/70 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.45)]"> <div className="mb-3 flex flex-wrap items-center justify-between gap-2"> <p className="text-xs font-medium uppercase tracking-wide text-white/50"> Gmail triage </p> <div className="flex items-center gap-1.5"> <button type="button" onClick={() => { window.location.href ="/api/gmail/oauth/start"; }} className="rounded-md border border-white/[0.08] bg-bip-page px-2 py-1 text-xs text-white/75 hover:bg-white/[0.06]" > {gmailConnected ?"Reconnect Gmail" :"Connect Gmail"} </button> <button type="button" onClick={() => void handleSyncGmail()} disabled={!gmailConnected || gmailLoading} className="rounded-md border border-white/[0.08] bg-bip-page px-2 py-1 text-xs text-white/75 hover:bg-white/[0.06] disabled:opacity-60" > {gmailLoading ?"Syncing..." :"Sync inbox"} </button> </div> </div> <p className="mb-2 text-[11px] text-white/50">{gmailConnectMeta ||"Not connected"}</p> <div className="mb-2 flex flex-wrap items-center gap-1.5"> <input value={gmailQuery} onChange={(event) => setGmailQuery(event.target.value)} placeholder="Search mail..." className="rounded border border-white/[0.12] bg-bip-card/85 px-2 py-1 text-xs text-white/75" /> {( [ ["inbox","Inbox"], ["needs_action","Needs action"], ["high_priority","High priority"], ["archived","Archived"], ["deleted","Deleted"], ] as const ).map(([value, label]) => ( <button key={value} type="button" onClick={() => { setGmailView(value); void reloadGmailMessages(value); }} className={`rounded border px-2 py-1 text-[11px] ${ gmailView === value ?"border-white/[0.12] bg-white/[0.12] text-white" :"border-white/[0.12] bg-bip-card/85 text-white/75 hover:bg-white/[0.06]" }`} > {label} </button> ))} </div> <div className="grid gap-2 lg:grid-cols-[1fr_1.2fr]"> <div className="max-h-64 overflow-auto rounded border border-white/[0.12] bg-bip-card/85"> {filteredGmailMessages.map((message, index) => ( <button key={message.id} type="button" onClick={() => setGmailMessageIndex(index)} className={`block w-full border-b border-white/[0.08] px-2 py-1.5 text-left text-xs hover:bg-bip-page ${ index === gmailMessageIndex ?"bg-bip-accent/10" :"" }`} > <p className="truncate font-medium text-white/75"> {message.subject ||"(no subject)"} </p> <p className="truncate text-white/50"> {message.from_name || message.from_email ||"Unknown sender"} </p> </button> ))} {filteredGmailMessages.length === 0 && ( <p className="px-3 py-6 text-center text-xs text-white/50"> No messages in this view. </p> )} </div> <div className="rounded border border-white/[0.12] bg-bip-card/85 p-2"> {!selectedGmailMessage ? ( <p className="text-xs text-white/50">Select a message to triage.</p> ) : ( <> <p className="text-xs font-semibold text-white/75"> {selectedGmailMessage.subject ||"(no subject)"} </p> <p className="mt-1 text-[11px] text-white/50"> From:{""} {selectedGmailMessage.from_name || selectedGmailMessage.from_email ||"Unknown"} </p> <p className="mt-1 text-[11px] text-white/50"> {selectedGmailMessage.internal_date ? new Date(selectedGmailMessage.internal_date).toLocaleString() :"No date"} </p> <p className="mt-2 max-h-24 overflow-auto whitespace-pre-wrap text-xs text-white/75"> {selectedGmailMessage.body_text || selectedGmailMessage.snippet ||"(empty message)"} </p> <div className="mt-2 flex flex-wrap gap-1.5"> <button type="button" onClick={() => void handleGmailAction("archive")} className="rounded border px-2 py-1 text-[11px] hover:bg-bip-page">Archive (A)</button> <button type="button" onClick={() => void handleGmailAction("trash")} className="rounded border px-2 py-1 text-[11px] hover:bg-bip-page">Delete (D)</button> <button type="button" onClick={() => void handleGmailAction("create_task")} className="rounded border px-2 py-1 text-[11px] hover:bg-bip-page">To task (T)</button> <button type="button" onClick={() => void handleGmailAction(selectedGmailMessage.is_read ?"mark_unread" :"mark_read")} className="rounded border px-2 py-1 text-[11px] hover:bg-bip-page">{selectedGmailMessage.is_read ?"Unread" :"Read"}</button> <button type="button" onClick={() => void handleGmailAction(selectedGmailMessage.is_starred ?"unstar" :"star")} className="rounded border px-2 py-1 text-[11px] hover:bg-bip-page">{selectedGmailMessage.is_starred ?"Unstar" :"Star"}</button> <button type="button" onClick={() => void handleGmailAction("set_high_priority", !selectedGmailMessage.is_high_priority)} className="rounded border px-2 py-1 text-[11px] hover:bg-bip-page">{selectedGmailMessage.is_high_priority ?"Unmark high" :"High priority (H)"}</button> <button type="button" onClick={() => { const sender = selectedGmailMessage.from_email;
...
if (!sender) return; void handleUpsertGmailRule(sender,"always_high_priority"); }} className="rounded border px-2 py-1 text-[11px] hover:bg-bip-page" > Always high sender </button> </div> <p className="mt-2 text-[11px] text-white/50"> Shortcuts: J/K navigate, A archive, D delete, T task, B blacklist, H high priority. </p> </> )} </div> </div> {gmailRules.length > 0 && ( <p className="mt-2 text-[11px] text-white/50"> Active sender rules:{""} {gmailRules .slice(0, 6) .map((rule) => `${rule.sender} (${rule.rule_type})`) .join(",")} </p> )} </div> <div className="mt-4 rounded-lg border border-white/[0.08] bg-bip-card/70 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.45)]"> <p className="text-xs font-medium uppercase tracking-wide text-white/50"> Import legacy table </p> <textarea value={importText} onChange={(event) => setImportText(event.target.value)} className="mt-2 min-h-24 w-full rounded border border-white/[0.12] bg-bip-card/85 px-2 py-1.5 font-mono text-xs text-white/75" placeholder="Paste tab-delimited rows with header..." /> <div className="mt-2 flex items-center gap-2"> <button type="button" onClick={() => void handleImportLegacyTasks()} disabled={importing} className="rounded-md border border-white/[0.08] bg-bip-page px-3 py-1.5 text-sm text-white/75 hover:bg-white/[0.06]" > {importing ?"Importing..." :"Import tasks"} </button> {importMessage && <span className="text-xs text-white/50">{importMessage}</span>} </div> </div> </section> ) : null} {error && ( <p className="mt-3 rounded-md border border-bip-danger/30 bg-bip-danger/10 px-3 py-2 text-sm text-bip-danger"> {error} </p> )} </main> </div> {detailTask && ( <> <button type="button" className="fixed inset-0 z-40 bg-black/35" onClick={closeDetail} aria-label="Close task details" /> <aside className="fixed inset-y-0 right-0 z-50 w-full max-w-xl overflow-y-auto border-l border-white/[0.08] bg-bip-card p-5 shadow-2xl transition-transform"> <div className="flex items-start justify-between gap-2"> <div> <p className="text-xs uppercase tracking-wide text-white/50">Task detail</p> <h2 className="text-base font-semibold text-white/75">{detailTask.title}</h2> </div> <button type="button" onClick={closeDetail} className="rounded-md border border-white/[0.12] px-2 py-1 text-xs hover:bg-white/[0.06]" > Close </button> </div> <section className="mt-4 space-y-2"> <p className="text-xs font-semibold uppercase tracking-wide text-white/50">Description</p> <textarea value={detailDescription} onChange={(event) => setDetailDescription(event.target.value)} className="min-h-28 w-full rounded-lg border border-white/[0.12] px-3 py-2 text-sm text-white/75" /> <button type="button" onClick={() => void handleSaveDetailDescription()} disabled={detailSavingDescription} className="rounded-md border border-white/[0.12] bg-bip-card px-3 py-1.5 text-xs hover:bg-white/[0.06]" > {detailSavingDescription ?"Saving..." :"Save description"} </button> </section> <section className="mt-6 space-y-2"> <p className="text-xs font-semibold uppercase tracking-wide text-white/50">Task settings</p> <div className="grid grid-cols-2 gap-2"> <select value={detailStatus} onChange={(event) => setDetailStatus(event.target.value as UserTaskStatus)} className="rounded border border-white/[0.12] px-2 py-1.5 text-xs" > <option value="not_started">Not Started</option> <option value="in_progress">In Progress</option> <option value="waiting_on_client">Waiting on Client</option> <option value="done">Done</option> </select> <select value={detailPriority} onChange={(event) => setDetailPriority(event.target.value as UserTaskPriority)} className="rounded border border-white/[0.12] px-2 py-1.5 text-xs" > <option value="high">High priority</option> <option value="medium">Medium priority</option> <option value="low">Low priority</option> </select> <input type="date" value={detailDueDate} onChange={(event) => setDetailDueDate(event.target.value)} className="rounded border border-white/[0.12] px-2 py-1.5 text-xs" /> <select value={detailCategoryId} onChange={(event) => setDetailCategoryId(event.target.value)} className="rounded border border-white/[0.12] px-2 py-1.5 text-xs" > <option value="">Label (category)</option> {categories.map((category) => ( <option key={category.id} value={category.id}> {category.name} </option> ))} </select> <select value={detailClientId} onChange={(event) => { setDetailClientId(event.target.value);
setDetailProjectId("");
setDetailPhaseId(""); }} className="rounded border border-white/[0.12] px-2 py-1.5 text-xs" > <option value="">Client (optional)</option> {clientOptions.map((client) => ( <option key={client.id} value={client.id}> {client.account_name} </option> ))} </select> <select value={detailProjectId} onChange={(event) => { setDetailProjectId(event.target.value);
setDetailPhaseId(""); }} className="rounded border border-white/[0.12] px-2 py-1.5 text-xs" > <option value="">Project (optional)</option> {detailProjectOptions.map((project) => ( <option key={project.id} value={project.id}> {project.name} · {projectClientLabel(project.client)} </option> ))} </select> <select value={detailPhaseId} onChange={(event) => setDetailPhaseId(event.target.value)} disabled={!detailProject} className="rounded border border-white/[0.12] px-2 py-1.5 text-xs disabled:opacity-50" > <option value="">Phase (optional)</option> {(detailProject?.phases ?? []).map((phase) => ( <option key={phase.id} value={phase.id}> {phase.title} </option> ))} </select> </div> <div className="flex items-center gap-2"> <input value={detailNewCategoryName} onChange={(event) => setDetailNewCategoryName(event.target.value)} placeholder="Add new label" className="flex-1 rounded border border-white/[0.12] px-2 py-1.5 text-xs" /> <button type="button" onClick={() => void handleAddCategory()} disabled={addingCategory} className="inline-flex items-center gap-1 rounded border border-white/[0.12] px-2 py-1.5 text-xs hover:bg-white/[0.06]" > {addingCategory ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <FolderPlus className="h-3.5 w-3.5" />} Add label </button> </div> <button type="button" onClick={() => void handleSaveDetailMeta()} disabled={detailSavingMeta} className="rounded-md border border-white/[0.12] bg-bip-card px-3 py-1.5 text-xs hover:bg-white/[0.06]" > {detailSavingMeta ?"Saving..." :"Save settings"} </button> </section> <section className="mt-6 space-y-2"> <p className="text-xs font-semibold uppercase tracking-wide text-white/50">Assignees</p> <div className="grid grid-cols-2 gap-2"> {people.map((person) => ( <label key={person.id} className="inline-flex items-center gap-2 rounded border border-white/[0.12] px-2 py-1.5 text-xs" > <input type="checkbox" checked={detailSelectedPersonIds.includes(person.id)} onChange={(event) => setDetailSelectedPersonIds((prev) => event.target.checked ? [...new Set([...prev, person.id])] : prev.filter((id) => id !== person.id), ) } /> {person.name} </label> ))} </div> <button type="button" onClick={() => void handleSaveAssignees()} disabled={detailSavingAssignees} className="rounded-md border border-white/[0.12] bg-bip-card px-3 py-1.5 text-xs hover:bg-white/[0.06]" > {detailSavingAssignees ?"Saving..." :"Save assignees"} </button> </section> <section className="mt-6 space-y-2"> <p className="text-xs font-semibold uppercase tracking-wide text-white/50">Links</p> <div className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]"> <input value={detailLinkLabel} onChange={(event) => setDetailLinkLabel(event.target.value)} placeholder="Label" className="rounded border border-white/[0.12] px-2 py-1.5 text-xs" /> <input value={detailLinkUrl} onChange={(event) => setDetailLinkUrl(event.target.value)} placeholder="https://..." className="rounded border border-white/[0.12] px-2 py-1.5 text-xs" /> <button type="button" onClick={() => void handleAddLink()} disabled={detailSavingLink} className="rounded border border-white/[0.12] px-2 py-1.5 text-xs hover:bg-white/[0.06]" > {detailSavingLink ?"Saving..." :"Add"} </button> </div> <ul className="space-y-1"> {detailTask.links.map((link) => ( <li key={link.id} className="flex items-center justify-between rounded border border-white/[0.12] px-2 py-1.5 text-xs"> <a href={link.url} target="_blank" rel="noopener noreferrer" className="truncate text-white/75 underline" > {link.label} </a> <button type="button" onClick={() => void handleDeleteLink(link.id)} className="text-red-600" > Remove </button> </li> ))} </ul> </section> <section className="mt-6 space-y-2"> <p className="text-xs font-semibold uppercase tracking-wide text-white/50">Attachments</p> <label className="inline-flex cursor-pointer items-center gap-2 rounded border border-white/[0.12] px-2 py-1.5 text-xs hover:bg-white/[0.06]"> {detailUploadingAttachment ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Mail className="h-3.5 w-3.5" />} Upload file <input type="file" className="hidden" onChange={(event) => { const file = event.target.files?.[0];
if (file) {
  void handleUploadAttachment(file); event.currentTarget.value =""; } }} /> </label> <ul className="space-y-1"> {detailTask.attachments.map((attachment) => ( <li key={attachment.id} className="flex items-center justify-between rounded border border-white/[0.12] px-2 py-1.5 text-xs"> <span className="truncate">{attachment.file_name}</span> <div className="inline-flex gap-2"> <button type="button" onClick={() => void handleOpenAttachment(attachment.storage_path)} className="underline" > Open </button> <button type="button" onClick={() => void handleDeleteAttachment(attachment.id)} className="text-red-600" > Remove </button> </div> </li> ))} </ul> </section>
<section className="mt-6 space-y-2">
  <p className="text-xs font-semibold uppercase tracking-wide text-white/50 flex items-center gap-1.5">
    <MessageCircle className="h-3.5 w-3.5" /> Ask AI about this task
  </p>
  <div className="max-h-64 overflow-y-auto space-y-2 rounded-lg border border-white/[0.08] bg-bip-card/50 p-2">
    {aiChatMessages.length === 0 ? (
      <p className="text-[12px] text-white/40 px-1">Ask Claude anything about this task — next steps, how to approach it, draft an email, think through a blocker...</p>
    ) : (
      aiChatMessages.map((msg, i) => (
        <div key={i} className={`rounded-md px-2.5 py-2 text-[13px] leading-relaxed ${msg.role === "user" ? "bg-bip-accent/15 text-white/80 ml-4" : "bg-white/[0.06] text-white/75 mr-4"}`}>
    ...
    {aiChatLoading && <div className="flex items-center gap-2 px-2.5 py-2 text-[13px] text-white/40"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Thinking...</div>}
  </div>
  <div className="flex gap-2">
    <input
      ...
      className="flex-1 rounded-md border border-white/[0.12] bg-bip-card/85 px-3 py-2 text-[13px] text-white/75 placeholder:text-white/30 disabled:opacity-60"
    />
    <button
      type="button"
  ...
  {chatConfirm && (
    <div className="flex items-center justify-between rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2">
      ...
      <span className="flex gap-2">
        ...
        <button type="button" onClick={() => setChatConfirm(false)} className="text-[12px] text-white/40 hover:text-white/70">Cancel</button>
      </span>
    </div>
  )}
  {aiChatMessages.length > 0 && (
    <button type="button" onClick={() => setAiChatMessages([])} className="text-[11px] text-white/30 hover:text-white/60">Clear conversation</button>
  )}
</section>
</aside> </> )} </div> );
}
