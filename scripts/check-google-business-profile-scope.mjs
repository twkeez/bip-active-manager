...
async function main() {
  ...
  const hasBusinessManageScope = scopes.includes(requiredScope);
  ...
  console.log("\nScope checks:");
  console.log(
    `- Google Business Profile (business.manage): ${hasBusinessManageScope ? "YES" : "NO"}`,
  );

  if (!hasBusinessManageScope) {
    console.log(
      "\nAction needed: regenerate GOOGLE_OAUTH_REFRESH_TOKEN including scope https://www.googleapis.com/auth/business.manage",
    );
    process.exitCode = 2;
  }
}
...
main().catch((error) => {
  console.error("Failed to check Google Business Profile OAuth scope.");
