...
drop policy if exists "auth users can read template config" on report_template_config;
drop policy if exists "auth users can upsert template config" on report_template_config;
...
drop policy if exists "auth users can manage client report configs" on client_report_configs;
