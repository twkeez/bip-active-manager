  ...
  try {
    ...
    const { data: artifact, error } = await supabase
      ...
      .insert({
        ...
        content_json: { prompt: body.prompt ?? null },
      })
      .select("*")
      .single();
    if (error || !artifact) {
      throw new Error(error?.message ?? "Failed to save brainstorm artifact");
    }
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Failed to generate brainstorm",
      },
      { status: 500 },
    );
  }
