...
import { KICKOFF_BLOCK_KEYS, type KickoffBlock } from "@/lib/clients/onboarding-kickoff";

const VALID_KEYS = new Set<string>(KICKOFF_BLOCK_KEYS);
...
export async function GET() {
  ...
  return NextResponse.json({ blocks: (data ?? []) as KickoffBlock[] });
}
...
export async function PUT(request: Request) {
  ...
  const { data, error } = await admin
    ...
    .order("sort_order", { ascending: true });
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ ok: true, blocks: (data ?? []) as KickoffBlock[] });
}
