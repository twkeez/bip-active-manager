...
import ReportDraftEditor from "@/components/reports/report-draft-editor";
...
export default async function ReportDraftPage({ params }: { params: Params }) {
  ...
  const {
    adsSnapshot, adsSignals, gscPageMetrics, gscSignals, gscQueryMetrics,
    socialDailySnapshots: socialDailyRows, socialSignals, seoCrawlIssues: crawlIssues,
    seoCrawlSnapshot: crawlSnapshot, lighthouseSnapshot: lighthouse,
    sitemapSnapshot, gbpSnapshot, gbpReviews,
  } = workspace;
  ...
  const kpis = buildReportingKpis({
    adsSnapshot, gscPageMetrics, gscSignals,
    gscSnapshotUpdatedAt: workspace.gscSnapshot?.updated_at ?? gscSignals[0]?.created_at ?? null,
    socialDailyRows, socialPostCount: workspace.socialPostSnapshots.length,
  ...
  const staleSourceCount = freshness.filter((i) => i.source !== "ga4" && i.status !== "fresh").length;
  ...
  const keywordRows = managedKeywords.map((target) => {
    ...
    const currentPosition = current.length > 0
      ? current.reduce((s, r) => s + r.position, 0) / current.length : null;
    const previousPosition = previous.length > 0
      ? previous.reduce((s, r) => s + r.position, 0) / previous.length : null;
    const positionDelta = currentPosition == null || previousPosition == null
      ? 0 : currentPosition - previousPosition;
    return {
      keyword: target.keyword, page_url: null,
      current_position: currentPosition, previous_position: previousPosition,
      position_delta: positionDelta, current_clicks: currentClicks,
      previous_clicks: previousClicks, current_impressions: currentImpressions,
      previous_impressions: previousImpressions, dropped_by_3_plus: positionDelta >= 3,
    } satisfies KeywordHealthRow;
  }).sort((a, b) => b.current_impressions - a.current_impressions);

  const report = buildClientReportModel({
    client, generatedAt: new Date().toISOString(),
    reportingWindowLabel: "Last 30 days",
    urgencyScore, kpis, alerts, freshness, actions, keywordRows,
    socialDailyRows, adsSnapshot, gscPageMetrics: gscPageMetrics as GscPageMetric[],
    gscQueryMetrics: queryRows, managedKeywords,
  ...
  const existingDraft: ReportDraft | null = draftRow ?? null;
